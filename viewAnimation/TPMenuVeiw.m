...
+ (instancetype)TPMenuViewInitialWithFrame:(CGRect)frame Blocks:(NSArray *)blocks deleController:(UIViewController *)vc keyWindow:(UIWindow *)kWindow{
    
    
//    .frame;
    
    
    TPMenuVeiw *menuView = [[TPMenuVeiw alloc]initWithFrame:frame];
    menuView.containerView = vc.view;
    [menuView.containerView addSubview:menuView];

    [menuView start:CGRectMake([UIScreen mainScreen].bounds.size.width - 70, [UIScreen mainScreen].bounds.size.height - 150, 50, 50)];

    menuView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:.2];
    menuView.isHide = YES;
...
//    self.mainImage.center = self.center;
    ...
    [self.containerView addSubview:_mainImage];
    ...
    [self.containerView addSubview:_favorateImage];
    ...
    [self.containerView addSubview:_loveImage];
    ...
    [self.containerView addSubview:_scanImage];
    ...
    [self.containerView addSubview:_addImage];
    ...
    for (int i = 0; i < 5; i ++) {
        UIView *view = [self.containerView viewWithTag:i + 100];
        view.alpha = 0;
        ...
        if (i == 0 ) {
            ...
            UIView *nextview = [self.containerView viewWithTag:100];
            view.center = nextview.center;
            continue;
        }
        UIView *nextview = [self.containerView viewWithTag:i + 101];
        view.center = nextview.center;
